...
def report(root: Optional[str] = None, *, quick: bool = False) -> Dict[str, Any]:
    ...
    def check(
        name: str, ok: bool, detail: str, *, blocking: bool = False, warn: bool = False
    ) -> None:
        out["checks"].append(
            {"name": name, "ok": bool(ok), "detail": detail, "blocking": bool(blocking)}
        )
        if not ok:
    ...
    check(
        "file-locking",
        caps.fcntl_lock,
        "fcntl.flock available"
        if caps.fcntl_lock
        else "falling back to exclusive-create lock files",
        warn=not caps.fcntl_lock,
    )
    check(
        "linux-host",
        caps.is_linux,
    ...
    # Remote mode is optional: it needs an ssh client and a declared target, and
    # the probe itself needs nothing on the target except a POSIX shell.
    # `util.which` keeps this accurate even under --quick, where the tool probe
    # is skipped.
    ssh_present = bool(caps.tools.get("ssh")) or util.which("ssh") is not None
    check(
        "remote-over-ssh",
        ssh_present,
        "ssh present: `ctfctl remote probe <host>` works on declared targets"
        if ssh_present
        else "ssh client missing: remote mode is unavailable until openssh-client is installed",
        warn=not ssh_present,
    )
    ...
    check(
        "profiles",
        not profile_problems,
        f"{len(profiles_mod.load_all(root))} profile(s) loaded and valid"
        if not profile_problems
        else " | ".join(profile_problems),
    )
    tested = [
        p.profile_id
        for p in profiles_mod.load_all(root)
        if p.support_level == "tested-auto"
    ]
    check(
        "tested-profiles",
        bool(tested),
        f"fast-apply tested: {', '.join(tested)}"
        if tested
    ...
    check(
        "fixtures",
        bool(names),
        f"{len(names)} fixture(s): {', '.join(names[:8])}"
        if names
        else "no fixtures present",
        warn=not names,
    )
    ...
    check(
        "target-declarations",
        declared > 0,
        f"{declared} declared target(s)"
        if declared
        else "no team-owned target declared: mutations are blocked, read-only work is unaffected",
        warn=declared == 0,
    )
    check(
        "event-policy",
        ...
        if acknowledged
        else "event policy not acknowledged in state/policy.json (required for host mutations on "
        "declared targets)",
...
def render(report_data: Dict[str, Any]) -> str:
    ...
    lines.append(
        f"python    {caps['python_version']}   sqlite {caps['sqlite_version']}"
    )
    lines.append("")
    lines.append("checks")
    ...
    lines.append(
        f"  read-only discovery        {'yes' if summary['ready_for_readonly'] else 'no'}"
    )
    lines.append(
        f"  knowledge-base search      {'yes' if summary['ready_for_kb_search'] else 'no'}"
    )
    lines.append(
        f"  ranked (FTS5) search      {'yes' if summary['ready_for_ranking'] else 'no (literal only)'}"
    )
    lines.append(
        f"  host mutation              {'yes' if summary['ready_for_mutation'] else 'no'}"
    )
    lines.append(
    ...
    lines.append("")
    lines.append(
        "This report performs no network access and installs nothing. Run "
