...
| CI | GitHub-hosted runners (ubuntu-latest, windows-latest); workflow run 35198144125 (current head `81d98f6`); earlier pre-normalization run 35191029558 |
...
Result (2026-09-17, latest): **236 passed, 0 failed, 2 modules skipped**, exit
...
reason (4 tests total); the current ubuntu CI run exercised both (see
"Continuous integration"). The module counts below sum to 240 with both
Docker-backed modules included; the 4 tests inside the two skipped modules ran
on ubuntu CI, and 236 ran in the local run above.
...
| `t_remote` | 42 | Host injection refusal, ssh argv construction, bundle contents, deterministic bundle bytes (patched advancing clock; zeroed gzip MTIME header), fingerprint change, probe parsing (ss/netstat/proc), declaration gate before ssh, two-pass plan, policy gate, `--yes` gate, apply mirroring, install upload (fingerprint mismatch, force re-upload, failure handling), verify execution over the fake transport, recover end to end, human-output regressions for recover and rollback, flag absorption for `remote run`, read-only allowlist, unreachable-host message |
...
**35198144125** (head `81d98f6`) is all green on every job:

| Runner | Result | Notes |
|---|---|---|
| ubuntu-latest | 240 passed, 0 failed, 0 skipped | Docker available; both container modules ran (`t_integration_docker` 3 ok, `t_integration_lockdown` 1 ok) |
| windows-latest | 236 passed, 0 failed, 2 modules skipped | The runner's Docker was in Windows-container mode; the fixture images need Linux containers |

This is the first all-green run on the normalized history and covers the
remote-test additions in the module table above, including the deterministic
bundle tests. The earlier pre-normalization record, run **35191029558**, is kept
for history:
...
* Recorded rehearsal (2026-09-17, head `81d98f6`; later commits, including this
  documentation update, contain only documentation changes): exit 0, version
  0.2.0, 219 files archived; `dist/ctf-buddy-0.2.0.tar.gz` is 619,491 bytes with
  sha256
  `126e19cf3096c7e025f6b196817628f5d22559ccd63942656b332748a362e0c1`, alongside
  ...
  licence and reuse status). This supersedes the two earlier records: the
  2026-09-17 interim rehearsal (commit `69a4e0f`, 618,104 bytes, sha256
  `d0298bc76bdca98046c0aca0c81c863c4f8d7020799546987b8dd3faf7247aa8`) and the
  2026-09-16 record (commit `8f689e3`, 612,577 bytes, sha256
...
* The rehearsal extracts the archive to a temporary directory and runs the full
  suite from there. Inside the archived tree the suite gave 236 passed, 0
...
|---|---|---|
| Docker-backed integration modules on the local Windows host (2026-09-17 run) | No Docker daemon was running | Start Docker Desktop and run `python tests/run_tests.py integration_docker`; the ubuntu CI jobs ran both modules in run 35198144125 |
